...
export default function Checkout() {
  ...
  // Load Midtrans Snap Script & Fetch Package Details
  useEffect(() => {
    const script = document.createElement('script');
    // Using sandbox or production URL based on env
    const isProduction = import.meta.env.VITE_MIDTRANS_IS_PRODUCTION === 'true';
    script.src = isProduction ? 'https://app.midtrans.com/snap/snap.js' : 'https://app.sandbox.midtrans.com/snap/snap.js';
    
    // Using Client Key from environment variables
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      // Bypass Midtrans for mock tokens (local dev without real Midtrans credentials)
      if (snapToken.startsWith('mock_')) {
        try { await api.post(`/orders/${orderToken}/simulate`); } catch(e){}
          ...
          <div className="glass-card rounded-2xl p-6 border border-white/10 mb-6">
            ...
            <form id="checkoutForm" onSubmit={handlePayment} className="space-y-4">
              <div>
                ...
                <input type="text" name="customer_name" required className="w-full bg-slate-900/50 border border-white/10 text-white px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 outline-none transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">Email Address</label>
                <input type="email" name="customer_email" required className="w-full bg-slate-900/50 border border-white/10 text-white px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 outline-none transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">WhatsApp Number (For License Delivery)</label>
                <input type="text" name="customer_phone" required className="w-full bg-slate-900/50 border border-white/10 text-white px-4 py-2.5 rounded-lg focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 outline-none transition-all" />
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="glass-card rounded-2xl p-6 border border-white/10 sticky top-24">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">{packageDetails ? packageDetails.name : ''}</p>
              </div>
            ...
            <div className="mb-6 pt-4 border-t border-white/5">
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input type="text" name="coupon_code" form="checkoutForm" placeholder="Promo code" className="w-full bg-slate-900/50 border border-white/10 text-white pl-9 pr-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 outline-none transition-all text-sm uppercase" />
                </div>
            ...
            <button type="submit" form="checkoutForm" disabled={loading || !packageDetails} className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-4 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] flex items-center justify-center gap-2">
